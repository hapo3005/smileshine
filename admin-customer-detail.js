(() => {
  ...
  function appointmentRow(a){const price=servicePrice(a.service),status=A.STATUS_LABELS[a.status]||a.status;return `<div class="customer-history-row"><div><strong>${dateShort(a.date)} · ${a.time} Uhr</strong><small>${escapeHTML(a.service)} · ${a.duration} Min.</small></div><div><strong>${money(price)}</strong><small>${escapeHTML(a.payment||'Im Studio')} · ${escapeHTML(status)}</small></div></div>`}
  ...
  function renderCustomerDetail(id){
    ...
    $('[data-customer-new-appointment]',body).addEventListener('click',()=>{$('#customerDetailModal').close();A.openModal?.();setTimeout(()=>{const form=$('#appointmentForm');if(!form)return;form.elements.customerName.value=customer.name;form.elements.phone.value=customer.phone||'';form.elements.email.value=customer.email||''},0)});
