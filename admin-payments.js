(() => {
  ...
  function refreshAppointmentList(){
    $$('.appointment-card[data-id]').forEach(card=>{const a=A.db.appointments.find(x=>x.id===card.dataset.id);if(!a)return;const f=financials(a);let badge=$('.appointment-payment-badge',card);if(!badge){badge=document.createElement('button');badge.type='button';badge.className='appointment-payment-badge';const menu=$('.row-menu',card);menu?.before(badge)}badge.className=`appointment-payment-badge ${statusClass(f.status)}`;badge.textContent=f.status==='paid'?`✓ ${money(f.paid)}`:`${statusLabel(f.status)} · ${money(f.open)}`;badge.onclick=()=>openPaymentModal(a.id);const menu=$('.row-menu',card);if(menu&&!$('[data-payment-id]',menu)){const btn=document.createElement('button');btn.type='button';btn.dataset.paymentId=a.id;btn.title='Preis & Zahlung';btn.textContent='€';btn.onclick=()=>openPaymentModal(a.id);menu.prepend(btn)}})
  }
  function refreshDashboardRevenue(){
    const grid=$('#kpiGrid');if(!grid)return;const cards=$$('.kpi-card',grid);if(cards.length<4)return;const now=new Date(),paid=(A.db.appointments||[]).filter(a=>{const d=new Date(`${a.date}T12:00:00`);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()&&a.status!=='cancelled'}).reduce((sum,a)=>sum+financials(a).paid,0);const card=cards[3],label=$('.kpi-label',card),value=$('.kpi-value',card),foot=$('.kpi-foot>span:first-child',card);if(label)label.textContent='Umsatz · Monat';if(value)value.textContent=money(paid);if(foot)foot.textContent='Tatsächlich bezahlt';
  }
