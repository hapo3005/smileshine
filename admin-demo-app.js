(() => {
  ...
  async function decorate(api){
    ...
    if(settings&&!settings.querySelector('[data-demo-storage-card]')){
      ...
      const simMeta=simulation?`<span>${Number(simulation.customerCount||0)} Kunden · ${Number(simulation.totalAppointments||0)} Termine</span><span>ca. 3 Monate voraus</span>`:'';
