(() => {
  ...
  function migrateDescriptions(){
    let changed=false;
    (A.db.services||[]).forEach(s=>{if(s.description===undefined){s.description=defaults[s.name]||'';changed=true}});
  ...
  function ensureUI(){
    ...
    if(heading&&!$('[data-action="newService"]',heading)){
      const btn=document.createElement('button');btn.type='button';btn.className='primary-action';btn.dataset.action='newService';btn.textContent='＋ Neue Leistung';heading.appendChild(btn);
    }
    if(!$('#serviceModal')){
  ...
  function saveNewService(form){
    ...
    A.db.services.push({id:uid('service'),name,description,duration,price,deposit,active,createdAt:new Date().toISOString()});
  ...
  function deleteService(id){
    const s=A.db.services.find(x=>x.id===id);if(!s)return;
    const appointments=A.db.appointments.filter(a=>a.service===s.name).length;
    const message=appointments?`„${s.name}“ wirklich löschen? ${appointments} bestehende Termin${appointments===1?' bleibt':'e bleiben'} mit eingefrorenem Namen und Preis erhalten.`:`„${s.name}“ wirklich löschen?`;
    if(!confirm(message))return;
    A.db.services=A.db.services.filter(x=>x.id!==id);A.addActivity('setting',`Leistung gelöscht: ${s.name}.`);A.save(`${s.name} wurde gelöscht.`);A.renderServices?.();
  }

  function initServiceManager(){
    ensureUI();A.bindServiceActions=bindServiceActions;
    const grid=$('#servicesGrid');if(grid)new MutationObserver(()=>{decorateCards();bindServiceActions()}).observe(grid,{childList:true});
    const baseShow=A.showView;A.showView=name=>{baseShow(name);if(name==='services'){ensureUI();decorateCards();bindServiceActions()}};
