(() => {
  ...
  window.SSAdmin=api;
  Promise.all([import('./admin-render.js'),import('./admin-actions.js'),import('./admin-calendar-views.js'),import('./admin-calendar-workspace.js?v=20260922-calendar-workspace2'),import('./admin-customer-detail.js'),import('./admin-payments.js'),import('./admin-appointment-detail.js'),import('./admin-services-manager.js'),import('./admin-customer-numbers.js?v=20260914-1443')]).then(()=>{api.initCustomerNumbers?.();api.bindActions();api.initServiceManager?.();api.initCalendarViews();api.renderAll();api.refreshPaymentUI?.();api.bindCustomerDetailRows?.();api.showView(location.hash.replace('#','')||'dashboard')}).catch(error=>{console.error(error);api.toast('Demo konnte nicht vollständig geladen werden.')});
})();