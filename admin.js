(() => {
  ...
  window.SSAdmin=api;
  Promise.all([import('./admin-render.js'),import('./admin-actions.js'),import('./admin-calendar-views.js'),import('./admin-calendar-workspace.js?v=20260922-calendar-workspace2'),import('./admin-customer-detail.js?v=20260922-customer-workspace2'),import('./admin-payments.js'),import('./admin-appointment-detail.js'),import('./admin-services-manager.js?v=20260922-customer-workspace3'),import('./admin-customer-numbers.js?v=20260922-customer-workspace1')]).then(()=>{api.initCustomerNumbers?.();api.bindActions();api.initServiceManager?.();api.initCalendarViews();api.renderAll();api.refreshPaymentUI?.();api.bindCustomerDetailRows?.();api.showView(location.hash.replace('#','')||'dashboard')}).catch(error=>{console.error(error);api.toast('Demo konnte nicht vollständig geladen werden.')});
})();