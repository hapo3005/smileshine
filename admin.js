(async () => {
  ...
  function seed(){
    ...
    return {version:1,slotInterval:30,buffer:15,
      ...
      activity:[{id:'x1',type:'booking',text:'Karin Hoffmann hat online einen Termin angefragt.',date:new Date().toISOString()},{id:'x2',type:'customer',text:'Neue Kundin in der Kartei: Julia Weber.',date:new Date(Date.now()-14400000).toISOString()},{id:'x3',type:'setting',text:'Arbeitszeiten wurden aktualisiert.',date:new Date(Date.now()-86400000).toISOString()}]};
  }
  ...
  window.SSAdmin=api;
  Promise.all([import('./admin-render.js?v=20260923-birgit-final2-adminflat2'),import('./admin-actions.js?v=20260925-communications5'),import('./admin-calendar-views.js?v=20260923-birgit-final2-adminflat2'),import('./admin-calendar-workspace.js?v=20260923-birgit-final2-adminflat2'),import('./admin-customer-detail.js?v=20260923-birgit-final2-adminflat2'),import('./admin-payments.js?v=20260923-birgit-final2-adminflat2'),import('./admin-appointment-detail.js?v=20260925-communications5'),import('./admin-services-manager.js?v=20260923-birgit-final2-adminflat2'),import('./admin-customer-numbers.js?v=20260923-birgit-final2-adminflat2'),import('./admin-pickup-shop.js?v=20260923-birgit-final2-adminflat2'),import('./admin-demo-profiles.js?v=20260923-birgit-final2-adminflat2'),import('./admin-whatsapp.js?v=20260925-communications5'),import('./admin-recurring-blocks.js?v=20260923-birgit-final2-adminflat2'),import('./admin-communications.js?v=20260925-communications5'),import('./admin-workflow.js?v=20260925-workfile1'),import('./admin-completion.js?v=20260925-communications5'),import('./admin-media.js?v=20260925-demoapp1')]).then(()=>{api.initDemoProfiles?.();api.initCustomerNumbers?.();api.bindActions();api.initServiceManager?.();api.initCalendarViews();api.initRecurringBlocks?.();api.initPickupShop?.();api.initCommunication?.();api.initWorkflowHub?.();api.renderAll();api.refreshPaymentUI?.();api.initWhatsApp?.();api.initCompletion?.();api.initCustomerMedia?.();api.bindCustomerDetailRows?.();window.SmileShineDemoApp?.init?.(api);api.showView(location.hash.replace('#','')||'dashboard');api.ready=true;document.documentElement.dataset.adminReady='true'}).catch(error=>{api.initError=String(error?.stack||error||'Unbekannter Initialisierungsfehler');console.error(error);api.toast('Die Studio-Vorschau konnte nicht vollständig geladen werden.')});
})();