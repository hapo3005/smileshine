(() => {
  ...
  function bindActions(){
    ...
    $('#resetDemo')?.addEventListener('click',()=>{if(!confirm('Demo-Daten wirklich zurücksetzen? Eigene lokale Änderungen gehen dabei verloren.'))return;A.db=A.seed();if(window.SmileShineDataStore)window.SmileShineDataStore.write(A.db);else localStorage.setItem(A.STORE_KEY,JSON.stringify(A.db));A.calendarCursor=new Date();A.renderAll();A.refreshPaymentUI?.();A.toast('Demo wurde zurückgesetzt.');A.showView('dashboard')});
