(() => {
  ...
  function bindActions(){
    ...
    $('#resetDemo')?.addEventListener('click',()=>{if(!confirm('Beispieldaten auf den vorbereiteten Ausgangszustand zurücksetzen? Eigene lokale Änderungen gehen dabei verloren.'))return;const base=window.SmileShineDataStore?.resetPresentationData?.()||A.seed();A.db=A.expandDemoProfiles?.(base)||base;localStorage.setItem(A.STORE_KEY,JSON.stringify(A.db));A.calendarCursor=new Date();A.renderAll();A.refreshPaymentUI?.();A.renderPickupOrders?.();A.toast('Beispieldaten wurden zurückgesetzt.');A.showView('dashboard')});
